...
const int numberOfMemoryLoc = 1e5;
...
template <class T>
void closeFile(string fileName, int size, vector<T> container,int start)
{
...
RegFile::RegFile()
{
    registers = vector<int>(numberOfRegisters, 0);
    ins = vector<int>(numberOfRegisters, -1);
}
...
{
    closeFile<int>("./Results/RegisterFile.txt",numberOfRegisters,registers,0);
}
...
{
    instrs = vector<string>(numberOfMemoryLoc + 10, string(32, '0'));
    ifstream in;
...
{
    closeFile<string>("./Results/InstructionMemory.txt",numberOfMemoryLoc,instrs,1);
}
...
{
    closeFile<int>("./Results/DataMemory.txt",numberOfMemoryLoc,data,1);
}
